...
const warrior_card_playing_load = preload("res://interface/cards/spectral_warrior/warrior_card_playing.tscn")
	...
	if mouse_in == true:
		...
		if Input.is_action_just_released("Souris_left"):
			if global_position.y<700:
				...
				if get_parent().get_parent().get_node("Card_playing").has_node("warrior_card_playing"):
					get_parent().get_parent().get_node("Card_playing").get_node("warrior_card_playing").card_added = true
				else:
					get_parent().get_parent().get_node("Card_playing").add_child(warrior_card_playing_load.instance())
			else:
				get_parent().card_dragged = false
