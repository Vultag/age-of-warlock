...
func _process(delta):
	...
	$warrior_icon/charge.scale.x = bar_load
	...
	if bar_load >= 1:
		...
		get_parent().get_parent().get_parent().add_child(warrior_red)
		warrior_red.position = get_parent().get_parent().get_parent().get_node("Portal_red").position
		var warrior_blue = warrior_blue_load.instance()
		get_parent().get_parent().get_parent().add_child(warrior_blue)
		warrior_blue.position = get_parent().get_parent().get_parent().get_node("Portal_blue").position
