...
func _process(delta):
	...
	$ranger_icon/charge.scale.x = bar_load
	...
	if bar_load >= 1:
		...
		get_parent().get_parent().get_parent().add_child(ranger_red)
		ranger_red.position = get_parent().get_parent().get_parent().get_node("Portal_red").position
		var ranger_blue = ranger_blue_load.instance()
		get_parent().get_parent().get_parent().add_child(ranger_blue)
		ranger_blue.position = get_parent().get_parent().get_parent().get_node("Portal_blue").position
