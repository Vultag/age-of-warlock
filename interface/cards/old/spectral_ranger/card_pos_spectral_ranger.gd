...
const ranger_card_playing_load = preload("res://interface/cards/spectral_ranger/ranger_card_playing.tscn")


func _process(_delta):

	if stop == false:
		if get_node("ranger_card"):
	...
	if mouse_in == true:
		...
		if Input.is_action_just_released("Souris_left"):
			if global_position.y<700:
				...
				if get_parent().get_parent().get_node("Card_playing").has_node("ranger_card_playing"):
					get_parent().get_parent().get_node("Card_playing").get_node("ranger_card_playing").card_added = true
				else:
					get_parent().get_parent().get_node("Card_playing").add_child(ranger_card_playing_load.instance())
			else:
				get_parent().card_dragged = false
