...
const card_magie_playing_load = preload("res://interface/cards/card_magie_playing.tscn")
...
func _process(_delta):
	...
	if mouse_in == true:
		...
		if Input.is_action_just_released("Souris_left"):
			if global_position.y<700 && get_parent().get_parent().get_node("Card_playing").get_child_count()>=1:
				if !get_parent().get_parent().get_node("Card_playing").spell_active.has(get_node("card_magie").get_node("icons").get_animation()):
					...
					get_parent().get_parent().get_node("Card_playing").add_child(card_magie_playing)
					...
					#print(get_node("card_magie").get_node("icons").get_animation())
					#_card_playing_added(card_magie_playing)
					queue_free()
				else:
					get_parent().card_dragged = false
					set_global_position(return_card)
			elif global_position.y<700 && get_parent().get_parent().get_node("Card_playing").get_child_count()==0:
					...
					card_magie_playing.add_to_group(get_node("card_magie").get_node("icons").get_animation())
					get_parent().get_parent().get_node("Card_playing").spell_active.insert( get_parent().get_parent().get_node("Card_playing").spell_active.size(), get_node("card_magie").get_node("icons").get_animation() )
					card_magie_playing.selected = true
					_activate_spell(card_magie_playing)
					#get_parent().get_parent().get_node("Card_playing").card_selected = card_magie_playing
					queue_free()
			else:
				get_parent().card_dragged = false
				set_global_position(return_card)
...
func _activate_spell(spell_card):
	if spell_card.is_in_group("fire"):
		var Wizard = get_parent().get_parent().get_parent().get_node("Wizard")
...
"""
func _card_playing_added(card):
	if get_parent().get_parent().get_node("Card_playing").slot1 == null:
		get_parent().get_parent().get_node("Card_playing").slot1 = card
	elif get_parent().get_parent().get_node("Card_playing").slot2 == null:
		get_parent().get_parent().get_node("Card_playing").slot2 = card
	elif get_parent().get_parent().get_node("Card_playing").slot3 == null:
		get_parent().get_parent().get_node("Card_playing").slot3 = card
	elif get_parent().get_parent().get_node("Card_playing").slot4 == null:
		get_parent().get_parent().get_node("Card_playing").slot4 = card
	elif get_parent().get_parent().get_node("Card_playing").slot5 == null:
		get_parent().get_parent().get_node("Card_playing").slot5 = card
"""
