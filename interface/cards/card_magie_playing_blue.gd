...
func _process(delta):
	...
	if get_parent().get_parent().get_parent().has_node(str(globals.playerJoinId)):
		Wizard = get_parent().get_parent().get_parent().get_node(str(globals.playerJoinId))
