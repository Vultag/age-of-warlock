...
func _process(delta):
	...
	"""
	if body_char == null:
		if $proj.is_colliding() == true:
			if $proj.get_collider().is_in_group("Character_red"):
				hit = true
	"""
	
	"""
	$proj.position = self.get_point_position(0)
	$proj.set_cast_to(self.get_point_position( get_point_count()-1 ))
	
	
	
	#$Area2D/CollisionShape2D.set_polygon(1)
	$proj.set_collide_with_bodies(true) 
	
	$proj.position = self.get_point_position( get_point_count()-1 )
	
	if hit == true:
		
		_ray_entered_body($proj.get_collider())
		
		#self.rotation_degrees=body_char.get_angle_to(self.position)
		#$proj.get_collider().get_collision_normal()
		#reflect = $proj.get_collider().get_collision_normal()
		#reflect=mouv.rotated(Vector2(self.get_point_position(get_point_count()-1).x,self.get_point_position(get_point_count()-2).x).angle_to_point($proj.get_collision_normal()))
	"""
...
func _on_lazer_beam_body_entered(body):
