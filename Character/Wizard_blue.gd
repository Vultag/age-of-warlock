...
const max_mouv = 170
var spd = 170
onready var tir_arcane_load = preload("res://projectiles/tir_arcane_blue.tscn")
...
func _take_damage(damage,knock):
	...
	#if is_network_master():
		rpc("_take_damage_local",damage,knock)
...
sync func setAnim(anim):
	$Wizard_arm/AnimationPlayer.play(anim)

puppet func disable_shield(Bool):
	$Wizard_arm/Shield/Shield_hitbox.disabled = Bool
...
master func _take_damage_local(damage,knock):
...
sync func _die():
	...
	if has_node("fire"):#has_fire == true:
		get_parent().get_node("Portal_blue").respawn_with_fire_blue = true
	if has_node("frost"):#has_frost == true:
		get_parent().get_node("Portal_blue").respawn_with_frost_blue = true
	get_parent()._PJ_respawn("BLUE")
