...
func _physics_process(_delta):
	...
	if is_network_master():
		...
		if body_check == true:
			if is_instance_valid(body_char):# && body_char.is_in_group("Character_red"):
				if abs(body_char.global_position.x-self.global_position.x) <= 580 && abs(body_char.global_position.y-self.global_position.y) <= 550:
...
func _on_shoot_check_body_entered(body):
	if is_network_master():
		if body_char == null:#body.is_in_group("Character_red") && body_char == null:
			_aim()
...
puppet func setHealth(health):
	if ADD_HEALTH > 0:
		$Ranger_spr/barre_vie/divine_shield.scale.x = (health/1)
	else:
		HEALTH = health
		vie.scale.x=HEALTH/1
...
"""
puppet func setAnim(anim_spr,anim_legs,anim_arm,arm_rotation,arm_spd,Spd,arm_scale):
	$Ranger_spr.play(anim_spr)
	$Ranger_spr/Ranger_legs.play(anim_legs)
	$Ranger_arm.play(anim_arm)
	$Ranger_arm.rotation = arm_rotation
	$Ranger_arm.set_speed_scale(arm_spd)
	$Ranger_spr.set_speed_scale(Spd)
	$Ranger_arm.scale = arm_scale
	#$block_display.play(block)
"""
...
sync func _tir_arrow():
	...
	arrow.mouv = Vector2(arrow.spd, 0).rotated(arrow.rotation)
	if ($Ranger_arm.rotation_degrees)*sign($Ranger_arm.rotation_degrees)>90:
		arrow.flip_v = 1
	else: 
		arrow.flip_v = 0
