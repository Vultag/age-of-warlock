...
onready var hitbox = get_node("Spearman_spr/attack_hitbox/hitbox")
...
func _process(delta):
	...
	if Input.is_action_just_pressed("O"):
		mouv.x=0
		attack_free = false
		$Spearman_spr.play("attack")
	if ($Spearman_spr.get_frame()==4 and $Spearman_spr.get_animation()=="attack"):hitbox.disabled=false
	if ($Spearman_spr.get_frame()==8):
		$Spearman_spr.play("static")
		attack_free = true
		body_check = false
